...
= I can't (using the shortcode) hide/show the phone/name fields =
...
2. Have a look around the forums for a similar topic as your problem may already be solved

3. How to test what is wrong. 

 ** Disable all your other plugins 
 ** Switch to the default WordPress theme. 
4. If the problem does not go away, open a support topic, if not, figure out which plugin is the cause of the problem and open a thread mentioning both plugins
5. Finally, if you decide to open a new thread.
